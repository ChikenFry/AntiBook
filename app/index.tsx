...
import { Plus, BookOpen, Layers } from 'lucide-react-native';
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        try {
          ...
          updateBookText(newBookId, data.text || "Unable to extract text.");
        } catch (err) {
          console.error("Extraction error:", err);
          updateBookText(newBookId, "Error extracting text natively via Python.");
        }
        setLoading(false);
      }
    } catch (error) {
  ...
  const renderBook = ({ item }: { item: any }) => (
    <View style={styles.bookCard}>
      ...
      <View style={styles.actionRow}>
        ...
        <Pressable 
          style={[styles.miniBtn, styles.miniBtnAlt, !item.text && styles.miniBtnDisabled]}
          onPress={() => item.text && router.push({ pathname: '/feed', params: { id: item.id } })}
        >
          <Layers color="#111" size={16} />
          <Text style={[styles.miniBtnText, {color: '#111'}]}>Feed</Text>
        </Pressable>
      </View>
    </View>
  );
